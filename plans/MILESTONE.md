...
- **Backend dependency hygiene is OUT of B1.** Fixing `mjt-pub-api` dependabot / dependency warnings is **not** part of this build; recommended as a separate, wholly-unrelated `fix/` branch off `main` (shared-platform work, independently valuable and revertable, and B1 should build against the backend as-is). No M1 step performs dependency remediation. Human-confirmed (2026-09-06): stays out of B1, tracked as a separate `fix/` branch off `main`; plan-scoped only, not recorded as a standing DECISIONS.md entry.
